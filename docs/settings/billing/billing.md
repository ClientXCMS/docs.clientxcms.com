...
**Invoice terms and conditions**<br />
...
- `LOCAL`: generate and retain the document in ClientXCMS without sending it to an external provider;
- `QONTO`: transmit through the [Qonto e-invoicing addon](/extensions/addons/qonto-einvoicing);
- `PENNYLANE`: transmit through the [Pennylane addon](/extensions/addons/pennylane).

Only select Qonto or Pennylane after configuring and testing the corresponding connector under **Settings > Billing**.

**Public-sector provider**<br />
Selects how invoices intended for French public entities are handled:

- `LOCAL`: keep public-sector documents local;
- `CHORUS-PRO`: transmit them through the [Chorus Pro addon](/extensions/addons/chorus-pro).
